...
Notre volonté ici est d'extraire le contenu /Domain Specific/ du code source comme le mondre la figure [[GPL-to-domain]]. L'accès aux services associés à un domaine devient possible sans développement spécifique, malgré l'utilisation de DSL interne.
...
- toutes les annotations de noeud sont prises en compte (les noeuds auquels nous nous intéressons sont les classes, les attributs et les méthodes)
...
#+CAPTION: Pseudocode transformant naïf l'ANT en méta-modèle
#+BEGIN_SRC ruby
...
Suite à l'analyse de l'ANT de la machine à état, Busimo infére le méta-modèle en figure [[State-machine-metamodel]]. Il est intéressant de pouvoir proposer une vision méta des modèles sous forme de code source. Cela permet de valider la conception du DSL créé à l'aide des annotations.
...
=Xtend=, comme =Java=, n'étant pas réflexif, le framework =EMF= (pour /Eclipse Modeling Framework/) est utilisé pour définir dynamiquement de nouvelles entités nécessaires à la définition du méta-modèle. Les modèles en sortie sont au format =XML=. Ils sont directement exploitable par l'environnment de développement intégré Eclipse grâce au /plugin/ =EMF=.

** Expériences

Les résultats que nous donne l'outil Busimo nous montre que des modèles peuvent être déduit du code source, notamment grâce aux annotations. On peut retrouver comme information le nom de l'annoation, le noeud sur lequel elle est placé. La place du noeud annoté est très intéressante pour Busimo. Ces imbriquations permettent de déduire des références entre les entités des méta-modèles générés.

Cela dit, il y a beaucoup d'impressisions dans les sources GPL. Certains éléments sont très problématiques. Par exemple, on peut trouver des annotations insérés aux même niveau dans le code source. Busimo ne peut créer de lien entre ces deux annotations. On peut en voir un exemple sur le méta-modèle généré en analysant du code de test en figure [[JUnit-metamodel]], où =@Test= et =@Ignore= se trouve sur un même noeud. =@Ignore= est en quelques sortes un attribut de =@Test=. Cela nous montre bien que certaines pratiques d'utilisation des annotions n'est pas conforme à nos stratégies de déduction. Ici, l'annotation qui aurait été préféré aurait été =@Test(ignored=true)=.
...
Nous ne pouvons pas blâmer certaines bibliothèque de proposer des annotations ne permettant pas à Busimo de générer des modèles intéressant : les annotations ne sont pas initialement utilisé dans ce but. Cependant, annoter un code source en connaissance de cause peut permettre à Busimo de générer des modèles très cohérents. On peut très bien imaginer Busimo comme un assistant de validation d'instance d'entités domaine lors de la phase de développement ou de relecture de code.

+ info exploitables dans le code source: les annotations!
  - notion de domaine (le nom de l'annotation)
  - imbriquation ok

+ trop d'impressision dans les annotations
  - éléments problématiques :oo
...
Les annotations peuvent avoir des paramétres. Ils ne sont pas pris en compte par Busimo. Malgré tout, cela donne une information qui, selon notre interprétation, sont des attributs des entités généré. Ces nouvelle donnée devrait être prise en compte.

** Système d'option et de journal

Busimo ne permet pas d'interaction avec l'utilisateur. L'interface homme-machine doit être développée. Cela est nécessaire notament pour implémenter les règles de filtrage ainsi que pour notifier l'utilisateur des choix effectué dans lors des déductions de Busimo. Tout ceci est à intégrer dans l'environnement de développement intégré Eclipse sous forme de /plugin/.
...
La qualité des déductions de Busimo sont évaluées manuellement avec du code source pré-sélectionné. De ce fait, nous ne pouvons dire que nous sommes très objectif sur l'intérêt de cette outil. Nous souhaitons cependant valider la pertinence de notre approche.

Pour ce faire, il est d'une part nécessaire de définir des critères de qualité. Une analyse plus formelle du résultat autoriserait l'automatisation de évalution. Quant au problème des entrées, une base de code doit être récupéré de manière non arbitraire. Github étant la plus grande forge de projet de développement aujourd'hui, il serait intéressant de s'y servir.
