#+TITLE: Modélisation inverse à l'aide des annotations
...
#+LATEX_HEADER: \abstract{Dans ce rapport de stage, nous produisons un état de l'art des langages dédiés (aussi appelés DSL pour \textit{Domain-Specific Languages}) à l'aide de la littérature scientifique sur le sujet. Nous définissons le DSL en exposant ses problématiques. La diversité des formes d'un même DSL étant avérée, la question du langage dédié métamorphique se pose. Deux études sont présentées ici : le cas du logiciel MPS, un \textit{Language Workbench} proposant une forme de métamorphisme, ainsi que le prototypage d'un outil de détection de domaine dans le code source.}
...
Si les DSL sont partout, ils possèdent souvent diverses formes pour un même domaine. La définition du langage dédié métamorphique \cite{Acher-et-al-2014} se base sur ce constat. Ce stage a permit de continuer les recherches dans ce sujet.

Commencant par une étudé bibliographique sur les langages dédiés, nous présenterons par la suite le cas MPS, logiciel de type /Language Workbench/ (c'est-à-dire, de création de langage formelle). Avant de conclure, nous poursuivrons avec la présentation de l'outil de détection de domaine dans le code source.

* Contexte
** Notions de modèle

Un *modèle* (relatif à un domaine) est la conceptualisation d'un sujet par rapport à une problématique précise (voir figure [[Two-Model-Example]]). Grâce à lui, on peut définir de manière plus ou moins abstraite un problème et/ou sa solution. Dans le cadre de la méthodologie MDE (/Model-driven engineering/ ou Ingénierie Dirigée par les Modèles), tout est considéré comme modèle, des schémas aux codes source. Selon la forme, un modèle équivalent au niveau domaine peuvent contenir plus ou moins d'informations.

#+NAME: Two-Model-Example
...
Littéralement modèle du modèle, un *metamodèle* est un modèle définissant les règles et les contraites de forme d'un autre modèle. Il définit donc un type de modèle. La figure [[Model-and-Metamodel-Exemple]] nous montre la différence entre un modèle et son méta-modèle.
...
** Modèles, langages et domaines

Deux approches permettent des transformations de formes. La première est la solution /ad hoc/ (voir figure [Ad-hoc-Ruby-to-Javascript-with-Opal]), la seconde stratégie est de passer par une representation intermédiaire (voir figure [Generic-Model-to-Model-Transformation]). La seconde est forcément la plus intéressante de par sa nature générique. Dans ce cas, une syntaxe abstraite centrale doit être définie, c'est-à-dire que l'intersection des domaines des différents modèles doit être spécifié (dans notre exemple, =XML=, =JSON=, =CSV= et =YAML= partagent des notions du domaines de la sérialisation de données).

#+NAME: Ad-hoc-Ruby-to-Javascript-with-Opal
#+CAPTION: Transformation /ad hoc/ Ruby vers Javascript avec [[http://opalrb.org/][Opal]]
#+ATTR_LATEX: :width 13cm
[ TODO ]

#+NAME: Generic-Model-to-Model-Transformation
#+CAPTION: Transformation générique grâce à une syntaxe abstraite
#+ATTR_LATEX: :width 13cm
[ TODO ]

Le cas des GPL est plus complexe que l'exemple des formats de sérialisation de données (qui sont des DSL). Les langages de programmation donnent la possibilité de définir un modèle pour un domaine. Il faut donc comprendre ici que le métier ne respecte pas le metamodèle du GPL mais celui défini par les modèles rédigé dans ce GPL. La figure [[Models-GPL-and-Domain]] montre ces relations.

#+NAME: Models-GPL-and-Domain
#+CAPTION: Rapport entre les modèles dans le GPL et les domaines
#+ATTR_LATEX: :width 15cm
[ TODO ]

La forme d'un code source (=M(gpl)=) est formalisé par son méta-modèle (=MM(gpl)=). Ce =M(gpl)= est une des manières de définir le méta-modèle du domaine ciblé (=MM(domain)=). Exécuter =M(gpl)= revient donc à modéliser =MM(domain)= ainsi qu'à instancier un modèle (=M(domain)=) conforme à ce =MM(domain)=. Ceci est d'autant plus clair dans les langages orientés-objet, les principe de classe étant la définition d'un type (niveau méta) auquel lui est associé ses attributs et ses comportements. Un exemple avec le cas de la bibliothèque =jOOQ= se trouve en figure [[jOOQ-Models]].

#+NAME: jOOQ-Models
#+CAPTION: Les modèles dans jOOQ
#+ATTR_LATEX: :width 15cm
[ TODO ]

L'UML est  intéressant pour tirer les notions domaines retrouvé dans le code source. Cependant, l'UML n'est qu'une forme supplémentaire pour représenter un modèle orienté objet. Le /Domain-Specific/ ne se retrouve pas nécessairement dans ce metamodèle. Si l'on reprends la figure [[Internal-DSL-Example]], on remarque que selon la stratégie utilisé pour émuler un DSL interne, ce n'est pas grâce à une représentation UML que l'on va pouvoir retrouver le domaine. Cette conclusion nous indique qu'il serait nécessaire d'analyser les AST pour en retirer du /Domain-Specific/.
...
L'outil Busimo a pour but d'inferrer un méta-modèle orienté domaine grâce aux annotations insérée dans les codes source. Il prend en entrée du code (=Xtend= uniqument à la date du document), en récupére la forme abstraite pour en créer un modèle arboressant des noeuds annotés (AnnotableNode.  Le schéma [[How-Busimo-Works]] montre le fonctionnement global de Busimo.
...
#+NAME: Multi-Source-Annotation-Picker
#+CAPTION: Analyser plusieur type de source
#+ATTR_LATEX: :width 15cm
[ TODO ]

Référence:
- \cite{Pawlak-et-al-2006}

** Règles de filtrage

Certaines annotations ne sont pas orienté domaine. Nous pouvons prendre l'exemple en Java des annotaion =@Override=, =@SuppressWarnings=, ou encore =@FunctionalInterface= [fn:Code-as-Domain].

** Miner Github

Github est sans doute la plus grande source centralisé de code source. Il est donc facile de récupérer un grand nombre de source =Java= et =Xtend=. Une analyse de ces sources nous permettrait de valider la cohérence des modèles généré par Busimo.
...
[fn:Code-as-Domain] Il y a effectivement un domaine associé à ces annotations, mais ce domaine est complément lié à la facette technique du développement
