...
** Notions de langage
   ...
   La *syntaxe concrète* est la représentation que l'on se fait du langage \cite{Fowler-2005}. On distingue deux types de syntaxe concrète : les syntaxes textuelles qui sont composées d'une suite de caractères et les syntaxes graphiques qui sont construites avec des éléments tel que des boîtes, des flèches, /etc./. Une *grammaire* est un ensemble de règles qui contraignent un langage. Ces règles décrivent comment former des « phrases » en fonction des « mots » rencontrés. Le langage BNF [fn:Yacc-and-Bison] est un exemple d'outil permettant la définition de grammaire \cite{Garshol-2008} (voir figure [[BNF-Example]]), mettant en évidence les symboles terminaux et non-terminaux de la syntaxe.
...
** Modèles, GPL et domaines
   ...
   Le rapport entre les modèles, leurs méta-modèles, le GPL et les domaines est assez particulier du fait que les GPL sont des langages permettant la modélisation. La figure [[Models-GPL-and-Domain]] montre ces relations.
   ...
   La forme d'un code source (=M(gpl)=) est formalisé par son méta-modèle (=MM(gpl)=). Ce =M(gpl)= est une manière de définir le méta-modèle du domaine ciblé (=MM(domain)=). Exécuter le code source revient à instancier un modèle (=M(domain)=) conforme à =MM(domain)=.

   L'exemple en figure [[JooQ-Models]] …
...
** /Model-to-model/, /Domain-Specific/ et GPL

   Deux approches permettent des transformations de modèle à modèle. La première est la solution /ad hoc/ (voir figure [Ad-hoc-Ruby-to-Javascript-with-Opal]), la seconde stratégie est de passer par une representation intermédiaire (voir figure [Generic-Model-to-Model-Transformation]). La seconde est forcément la plus intéressante de par sa nature générique. Dans ce cas, une syntaxe abstraite centrale doit être définie, c'est-à-dire que l'intersection des domaines des différents modèles doit être spécifié (dans notre exemple, =XML=, =JSON=, =CSV= et =YAML= partagent des notions du domaines de la sérialisation de données).
  ...
  Le cas des GPL est plus complexe que l'exemple des formats de sérialisation de données (qui sont des DSL). Les langages de programmation donnent la possibilité de définir un modèle pour un domaine. Il faut donc comprendre ici que le métier ne respecte pas le metamodèle du GPL mais celui défini par les modèles rédigé dans ce GPL.

  Différence entre volonté et réalité :

  #+NAME: Model-and-Metamodel-in-GPL-and-DSL
  #+CAPTION: Relation entre les modèles et metamodèles dans le GPL et le DSL
  #+ATTR_LATEX: :width 13cm
  [ TODO Schéma MM(gpl), M(gpl), MM(domain), M(domain) ]
...
** S'intéresser à l'AST : le cas des annotations
   ...
   Les annotations dans le code source sont des meta-données pouvant être ajoutés sur différents noeuds de l'AST (package, classes, méthodes, /etc/.). Du comportement peut leur être associé (voir l'exemple avec la persistence en Java grâce à l'aide des annotations \cite{Reed-2007}). Côté domaine métier, les annotations sont aussi une source d'information.
